...
void Graphics::SetNativeResolution(void)
{
...
void Graphics::Initialize(void)
{
	...
	ASSERT_SUCCEEDED(dxgiFactory->CreateSwapChainForHwnd(g_CommandManager.GetCommandQueue(), m_hwnd, &swapChainDesc, nullptr, nullptr, &s_SwapChain1));
...
#pragma region InitData
	...
	SamplerAnisoWrapDesc.Filter = D3D12_FILTER_MIN_MAG_MIP_LINEAR;
	SamplerAnisoWrapDesc.ComparisonFunc = D3D12_COMPARISON_FUNC_ALWAYS;
	SamplerAnisoWrapDesc.MaxAnisotropy = 1;
	...
	DepthStateReadWrite = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);
...
void Graphics::PreparePresentLDR(void)
{
	...
	ColorBuffer& UpsampleDest = g_DisplayPlane[g_CurrentBuffer];
	...
	else if (UpsampleFilter == kSharpening)
	{
		...
		float X = cos((float)SharpeningRotation / 180.0f * 3.14159f) * (float)SharpeningSpread;
		float Y = sin((float)SharpeningRotation / 180.0f * 3.14159f) * (float)SharpeningSpread;

...
const ManagedTexture* TextureManager::LoadFromFile( const std::wstring& fileName, bool sRGB )
{
	...
	std::string ex = FileUtility::GetExtension(fileName);
	...
	{
		Tex = LoadStbiFromFile(fileName, sRGB);
	}
	return Tex;
}
...
const ManagedTexture* TextureManager::LoadDDSFromFile( const std::wstring& fileName, bool sRGB )
{
	...
	int size;
	const unsigned char* ba = FileUtility::ReadTextureDDS(MakeStr(fileName), size);
	if (size == 0 || !ManTex->CreateDDSFromMemory( ba, size, sRGB ))
...
const ManagedTexture* TextureManager::LoadStbiFromFile( const std::wstring& fileName, bool sRGB )
{
	auto ManagedTex = FindOrLoadTexture(fileName);
	...
	const unsigned char * ba= FileUtility::ReadTextureStbi(MakeStr(fileName), imageWidth, imageHeight, channels);
	
	if (imageWidth* imageHeight> 0)
	{
		//ManTex->CreateTGAFromMemory( ba->data(), ba->size(), sRGB );
		ManTex->Create(imageWidth, imageHeight, sRGB ? DXGI_FORMAT_R8G8B8A8_UNORM_SRGB : DXGI_FORMAT_R8G8B8A8_UNORM, ba);
		ManTex->GetResource()->SetName(fileName.c_str());
	}
	else
		ManTex->SetToInvalidTexture();

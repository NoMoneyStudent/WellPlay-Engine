...
void ProcessNode(const aiScene* scene,GameObject* model, aiNode* node, aiMatrix4x4 translation);
void ProcessAnimation(const aiScene* scene);

set<string> bonestringlist;
...
void ProcessNode(const aiScene* scene, GameObject* parent, aiNode* node, aiMatrix4x4 translation)
{
	GameObject* myNode = new GameObject(node->mName.C_Str());
	if (parent != nullptr)
	{
		myNode->GetTransform()->SetParent(parent->GetTransform());
	}
	Transform* myTrans = myNode->GetTransform();
	aiVector3D aiS;
	aiVector3D aiT;
	aiQuaternion aiR;
	aiMatrix4x4 global = node->mTransformation*translation;
	node->mTransformation.Decompose(aiS, aiR, aiT);
	XMVECTOR vc;
	SetVertexVector(vc, aiT);
	myTrans->SetLocalPosition(vc);
	SetVertexVector(vc, aiS);
	myTrans->SetLocalScale(vc);
	SetQuater(vc, aiR);
	myTrans->SetLocalRotation(vc);
	...
	for (UINT i = 0; i < node->mNumMeshes; i++)
	{
		...
		{
			MeshRender* render = myNode->AddComponent<MeshRender>();
